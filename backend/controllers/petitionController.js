...
export const createPetition = async (req, res) => {
  try {
    ...
    const [result] = await pool.query(
      `INSERT INTO petitions (user_id, title, category, description, attachment)
       VALUES (?, ?, ?, ?, ?)`,
      [userId, title, category, description, attachment]
    );

    // petition_code is auto-generated by MySQL trigger
...
export const updatePetitionStatus = async (req, res) => {
  try {
    ...
    await pool.query(
      `UPDATE petitions SET status = ?, remarks = ? WHERE id = ?`,
